                ...
                if(isset($_POST['moedertaal'])) {
                    if(isset($_POST['geslacht'])) {
                        if(isset($_POST['leeftijd'])) {
                            if(isset($_POST['student_zorg'])) {
                                if(isset($_POST['werk_zorg'])) {
                                    try {
                                        ...
                                        $stream = fopen($_FILES['leeftijdsgenoot_opname']['tmp_name'], 'r+');
                                        $filesystem->writeStream(
                                            $_FILES['leeftijdsgenoot_opname']['name'],
                                            $stream
                                        );
                                        ...
                                        $stream = fopen($_FILES['oudere_opname']['tmp_name'], 'r+');
                                        $filesystem->writeStream(
                                            $_FILES['oudere_opname']['name'],
                                            $stream
                                        );
                                        if (is_resource($stream)) {
                                            fclose($stream);
                                        ...
                                        $line = [
                                            ...
                                            filter_input(INPUT_POST, 'student_zorg', FILTER_SANITIZE_STRING),
                                            filter_input(INPUT_POST, 'werk_zorg', FILTER_SANITIZE_STRING),
                                            $_FILES['leeftijdsgenoot_opname']['name'],
                                            $_FILES['oudere_opname']['name'],
                                        ...
                                        echo json_encode(['okay' => false, msg => $e->getMessage()]);
                                    }
                                } else {
                                    http_response_code(400);
                                    echo json_encode(['okay' => false, msg => 'werk_zorg']);
                                }
                            } else {
                                http_response_code(400);
                                echo json_encode(['okay' => false, msg => 'student_zorg_missing']);
                            }
                        } else {
                            http_response_code(400);
